...
couchstore_error_t couchstore_set_deletes(bool del);
...
struct bench_info {
    ...
    uint32_t max_logs; // Max logs per node in Dotori
    uint32_t retr_len; // Retrieve buffer size in Dotori 

    uint32_t g_commit_wait; // Group commit wait in Dotori
    uint32_t num_aio_workers; // Number of AIO workers in Dotori
    uint32_t max_outstanding; // Max outstanding AIO reqs in Dotori
    uint64_t bcache_size; // Size of index cache in Dotori
    uint64_t index_bsize; // Index blocksize in Dotori
    bool cache_kv_pairs; // Cache KV pairs or not in Dotori
    bool async_reads;
...
static int filesize_chk_term = 1;
    ...
    if(value_pop) {
        ...
        if (!doc->data.buf) {
            ...
            for (i=0;i<rnd_str_len;++i){
                doc->data.buf[i] = abt_array[rand_r(&seed) % abt_array_size];
            }
        }
        ...
        memset((uint8_t*)doc->data.buf + doc->data.size - 1, 0x0, 1);
    } else {
        doc->data.buf = NULL;
        doc->data.size = 0;
    }
...
void * pop_thread(void *voidargs)
{
    ...
    for (i=0;i<batchsize;++i) {
        docs[i] = NULL;
        infos[i] = NULL;
        _create_doc(binfo, i, &docs[i], &infos[i], true);
    }
    ...
    while(c < n) {
        ...
        for (i=c; (i<c+batchsize && i<n); ++i){
            _create_doc(binfo, i, &docs[i-c], &infos[i-c], true);
            user_data += docs[i-c]->data.size;
            counter++;
        }
        spin_lock(args->lock);
...
void population(Db **db, struct bench_info *binfo)
{
    ...
    counter = 0;
    couchstore_set_deletes(false);

    pop_file.open("pop.txt");
    assert(!pop_file.fail());
    ...
    for (i=0;i<=binfo->pop_nthreads;++i){
        ...
        if (i<binfo->pop_nthreads) {
            ...
            sprintf(namebuf, "POP%ld", i);
            pthread_setname_np(tid[i], namebuf);
        } else {
...
struct bench_shared_stat {
    std::atomic<uint64_t> op_count_read;
    std::atomic<uint64_t> op_count_write;
...
struct iterate_async_args {
    std::atomic<uint64_t> batchsize;
...
void * bench_thread(void *voidargs)
{
    ...
    unsigned int seed = args->id;

    DocInfo *rq_info = NULL;
    user_data.store(binfo->ndocs * binfo->bodylen.a);
    ...
    while(!args->terminate_signal) {
        ...
        op_w = args->b_stat->op_count_write.load(std::memory_order_relaxed);
        op_r = args->b_stat->op_count_read.load(std::memory_order_relaxed);
        ...
        } else if (binfo->batch_dist.type == RND_ZIPFIAN) {
        ...
        if (binfo->batch_dist.type == RND_LATEST) {
            // do nothing. just get value in get_random later
        }
        ...
        if (write_mode) {
            ...
            for (j=0;j<batchsize;++j){
                ...
                if(!binfo->rmw) {
                    _create_doc(binfo, r, &rq_doc, &rq_info, true);
                    ...
                    if(monitoring) {
                        ...
                        if(l_stat->samples[cur_sample] > max.load(std::memory_order_relaxed)) {
                            max.store(l_stat->samples[cur_sample], std::memory_order_relaxed);
                        }
                    }
                } else {
                    /*
                     * Don't populate a value when read-modify-writing.
                     */

                    _create_doc(binfo, r, &rq_doc, &rq_info, false);
                    err = couchstore_save_document_rmw(db[curfile_no], rq_doc,
                            rq_info, args->id);
                }
        ...
        if (monitoring && !binfo->async_reads) {
            gap = stopwatch_get_curtime(&sw_latency);
            l_stat->samples[cur_sample] = _timeval_to_us(gap);
        }

        if (write_mode) {
            args->b_stat->op_count_write.fetch_add(batchsize, std::memory_order_relaxed); 
            args->b_stat->batch_count.fetch_add(1, std::memory_order_relaxed);
        } else {
            args->b_stat->op_count_read.fetch_add(batchsize, std::memory_order_relaxed);
            args->b_stat->batch_count.fetch_add(1, std::memory_order_relaxed);
        }
    }
...
couchstore_error_t couchstore_set_max_logs(uint32_t max_logs);
couchstore_error_t couchstore_set_retrieve_length(uint32_t max_logs);
couchstore_error_t couchstore_set_g_commit_wait(uint32_t g_commit_wait);
couchstore_error_t couchstore_set_num_aio_workers(uint32_t num);
couchstore_error_t couchstore_set_max_outstanding(uint32_t num);
couchstore_error_t couchstore_set_bcache(uint64_t size);
couchstore_error_t couchstore_set_index_bsize(uint64_t size);
couchstore_error_t couchstore_set_cache_kv_pairs(bool cache);
...
{
    double percentile[7] = {0.01, 0.05, 0.50, 0.95, 0.99, 0.999, 0.9999};
    uint64_t i, pos, begin, end;
    double avg = 0;
    int num_percentiles = 7;

    if (l_stat->nsamples < 10000) {
        //printf("Only had %lu samples\n", l_stat->nsamples);
        //// more than 10000 samples are necessary
        //return;
        num_percentiles = 6;
    }
    // sort
    ...
    for (i=0; i<num_percentiles; ++i) { // screen: only 7 percentiles
        pos = l_stat->nsamples * percentile[i];
        printf("%d %s (%f%%)", (int)l_stat->samples[pos],
                               unit, percentile[i]);
        if (i+1 < num_percentiles) {
            printf(", ");
        } 
    }
    if (log && log_fp) { // log file: all percentiles
        for (i=1;i<100;++i){
            pos = l_stat->nsamples * i / 100;
            fprintf(log_fp, "%d %d\n", (int)i, (int)l_stat->samples[pos]);
        }
        pos = l_stat->nsamples * 0.999;
        fprintf(log_fp, "%f %d\n", (double)0.999, (int)l_stat->samples[pos]);
        if(num_percentiles == 7) {
            pos = l_stat->nsamples * 0.9999;
            fprintf(log_fp, "%f %d\n", (double)0.9999, (int)l_stat->samples[pos]);
        }
        fprintf(log_fp, "MAX %d\n", (int)l_stat->samples[l_stat->nsamples - 1]);
    }
}
...
void do_bench(struct bench_info *binfo)
{
    ...
    couchstore_set_bcache(binfo->bcache_size);
    couchstore_set_index_bsize(binfo->index_bsize);
    couchstore_set_cache_kv_pairs(binfo->cache_kv_pairs);
    // set compression option
    couchstore_set_compression(binfo->compression);

    couchstore_set_wal_size(binfo->fdb_wal);
    couchstore_set_max_logs(binfo->max_logs);
    couchstore_set_retrieve_length(binfo->retr_len);
    couchstore_set_num_aio_workers(binfo->num_aio_workers);
    couchstore_set_max_outstanding(binfo->max_outstanding);
    couchstore_set_g_commit_wait(binfo->g_commit_wait);
    ...
    if (binfo->initialize) {
        ...
        for (i=0; i<(int)binfo->pop_nthreads; ++i){
            if(i == 0) {
                sprintf(curfile, "%s", binfo->init_filename);
            }
           if(binfo->rmw) {
                couchstore_set_flags(0x100);
            }
    ...
    for (i=0;i<bench_threads;++i){
        ...
        for (j=0; j<(int)binfo->nfiles; ++j){
            sprintf(curfile, "%s", binfo->filename);
    ...
    while (i < (int)binfo->nbatches || binfo->nbatches == 0) {
        ...
        op_count_read = b_stat.op_count_read.load(std::memory_order_relaxed);
        op_count_write = b_stat.op_count_write.load(std::memory_order_relaxed);
        i = b_stat.batch_count.load(std::memory_order_relaxed);
        ...
        if (stopwatch_check_ms(&progress, print_term_ms)) {
            ...
            if (display_tick % filesize_chk_term == 0) {
                ...
                space_amp = (double) dbinfo->space_used / user_data.load();
                print_filesize_approx(dbinfo->space_used, fsize2);
                app_wa = dbinfo->app_wa;
            }

            // actual file size / live data size / space amp
            printf(" (SPACE:%f (%lu %lu) / APPWA:%f) ", 
                    space_amp, user_data >> 20,
                    dbinfo->space_used >> 20, app_wa);

            printf("\n");
            printf("READ LATENCIES:\n");
            _print_percentile(binfo, &l_read, "us", false);
            printf("\n");
    ...
    if (binfo->latency_rate) {
        if (binfo->nwriters) {
            ...
            _print_percentile(binfo, &l_write, "us", true);
        }
        if (binfo->nreaders + binfo->niterators) {
            lprintf("\nread latency distribution\n");
            _print_percentile(binfo, &l_read, "us", true);
        }
    }
    lprintf("\n");
...
void _print_benchinfo(struct bench_info *binfo)
{
    ...
    if (binfo->rmw) {
        lprintf("read-modify-write: true\n");
    } else {
        lprintf("read-modify-write: false\n");
    }

    lprintf("compaction interval: %d seconds\n",
            (int)binfo->compact_period);
    lprintf("max logs: %u\n",
            (int)binfo->max_logs);
    lprintf("retrieve length: %u\n",
            (int)binfo->retr_len);
    lprintf("num aio workers: %u\n",
            (int)binfo->num_aio_workers);
    lprintf("max outstanding: %u\n",
            (int)binfo->max_outstanding);
    lprintf("group commit wait: %u\n",
            (int)binfo->g_commit_wait);
    lprintf("bcache size (MB): %lu\n",
            binfo->bcache_size >> 20);
    lprintf("index blocksize: %lu\n",
            binfo->index_bsize);
    lprintf("cache KV pairs: %s\n",
            binfo->cache_kv_pairs ? "yes" : "no");
}
...
struct bench_info get_benchinfo(char* bench_config_filename)
{
    ...
    sprintf(dbname, "Dotori");
    sprintf(dbname_postfix, "dotori");
    ...
    binfo.bcache_size =
        iniparser_getint(cfg, (char*)"db_config:bcache_size_MB", 4096);
    binfo.bcache_size *= (1024*1024);

    binfo.index_bsize =
        iniparser_getint(cfg, (char*)"db_config:index_blocksize", 4096);

    str = iniparser_getstring(cfg, (char*)"db_config:cache_kv_pairs", (char*) "true");
    if (str[0] == 't' || str[0] == 'T' || str[0] == 'e' || str[0] == 'E') {
        // enabled
        binfo.cache_kv_pairs= 1;
    } else {
        binfo.cache_kv_pairs = 0;
    }
    ...
    binfo.max_logs = 
        iniparser_getint(cfg, (char*)"db_config:max_logs", 4);
    binfo.retr_len = 
        iniparser_getint(cfg, (char*)"db_config:retrieve_length", 4096);
    binfo.num_aio_workers = 
        iniparser_getint(cfg, (char*)"db_config:num_aio_workers", 3);
    binfo.max_outstanding = 
        iniparser_getint(cfg, (char*)"db_config:max_outstanding", 24);
    binfo.g_commit_wait = 
        iniparser_getint(cfg, (char*)"db_config:g_commit_wait", 50);
    str = iniparser_getstring(cfg, (char*)"db_config:async_reads", (char*)"false");
    if (str[0] == 't' || str[0] == 'T' || str[0] == 'e' || str[0] == 'E') {
        // enabled
        binfo.async_reads = 1;
    } else {
    ...
    if (str[0] == 'u') {
        ...
        fprintf(stderr, "Unsupported key distribution\n");
        exit(0);
    }
    ...
    str = iniparser_getstring(cfg, (char*)"operation:rmw",
            (char*)"false");
    binfo.rmw = (str[0]=='t')?(1):(0);
