...
void compaction_callback_test(bool multi_kv)
{
    ...
    fconfig.compaction_cb_mask = FDB_CS_COMPLETE;
    ...
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    s = fdb_compact_kvssd(dbfile);
    TEST_CHK(s == FDB_RESULT_SUCCESS);

    TEST_CHK(cb_args.completes == 1);
    fdb_close(dbfile);
...
void compact_with_reopen_test()
{
    ...
    fdb_commit(dbfile, FDB_COMMIT_MANUAL_WAL_FLUSH);
    fdb_get_file_info(dbfile, &info);
    TEST_CHK(info.doc_count == 199); // hbtrie_insert doesn't return old offset
...
void compact_upto_test(bool multi_kv)
{
   ...
   // insert documents of 0-4
    ...
    if (!multi_kv) {
        ...
        status = fdb_compact_upto_kvssd(dbfile,
                                  markers[r].marker);
        TEST_CHK(status == FDB_RESULT_SUCCESS);
        // create a snapshot
        ...
        TEST_CHK(status == FDB_RESULT_SUCCESS);
        // create a snapshot
        printf("Want to open a snapshot at %lu\n", markers[i].kvs_markers[r].seqnum);
...
void unlink_after_compaction_test()
{
    ...
    // compaction
    s = fdb_compact_kvssd(dbfile);
    TEST_CHK(s == FDB_RESULT_ESNAPSHOTOPEN);
...
void *db_compact_during_doc_delete(void *args)
{
    ...
    { // parent
        ...
        memleak_end();
        TEST_RESULT("db_compact_during_doc_delete");
        return NULL;
    }
                  ...
                  void *ctx)
{
    ...
    if (status == FDB_CS_COMPLETE && !args->done) {
        int i;
        int n = 10;
        char keybuf[256], bodybuf[256];
        fdb_status s;
...
void compaction_with_concurrent_transaction_test()
{
    ...
    fdb_kvs_open_default(txn_file, &txn, &kvs_config);
...
void compaction_with_concurrent_update_test()
{
    ...
    // 1. inserted at the end of compaction
    for (i=0; i<2; ++i) {
        sprintf(keybuf, "zzz%d", i);
        sprintf(bodybuf, "zzzvalue%d", i);
                             ...
                             void *ctx)
{
    ...
    if (status == FDB_CS_BEGIN) {
        TEST_CHK(false);
    } else if (status == FDB_CS_COMPLETE) {
        TEST_CHK(!kv_name);
...
void compact_deleted_doc_test()
{
    ...
    /*
        In ForestKV, we don't flush the WAL during compaction
        like ForestDB does when moving files (should we be?),
        so num_flushable here will be 21 (the WAL inserts from
        the callback)
    */

    s = fdb_compact_kvssd(dbfile);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    TEST_CHK(wal_get_num_flushable(dbfile->root->file) == 21);
...
void large_insert_compaction_test()
{
    TEST_INIT();
    memleak_start();
    _reset_kvssd("/dev/nvme0n1", "forestKV");
    ...
    fdb_shutdown();
    memleak_end();
    TEST_RESULT("large_insert_compaction_test test");
}
...
void compact_upto_twice_test()
{
    ...
    status = fdb_get_all_snap_markers(dbfile, &markers,
                                      &num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_compact_upto_kvssd(dbfile, markers[num_markers-1].marker);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_free_snap_markers(markers, num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    ...
    status = fdb_get_all_snap_markers(dbfile, &markers,
                                      &num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_compact_upto_kvssd(dbfile, markers[num_markers-2].marker);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_free_snap_markers(markers, num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
void wal_delete_compact_upto_test()
{
    ...
    status = fdb_compact_upto_kvssd(dbfile, markers[1].marker);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_free_snap_markers(markers, num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
void compact_upto_post_snapshot_test()
{
    ...
    status = fdb_get_all_snap_markers(dbfile, &markers,
                                      &num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_compact_upto_kvssd(dbfile, markers[5].marker);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_free_snap_markers(markers, num_markers);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
void compact_upto_overwrite_test(int opt)
{
    ...
    s = fdb_compact_upto_kvssd(db_file, markers[upto].marker);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
...
void compact_with_snapshot_open_read_test()
{
  TEST_INIT();
  memleak_start();
  _reset_kvssd("/dev/nvme0n1", "forestKV");
  ...
  char keybuf[256], bodybuf[256];
  fdb_file_handle *dbfile;
  fdb_kvs_handle *db, *snap_db, *snap_db2, *snap_db3;
  fdb_status s;
  fdb_config fconfig = fdb_get_default_config_kvssd();
  fdb_kvs_config kvs_config = fdb_get_default_kvs_config();
  struct cb_args cb_args;
  ...
  s = fdb_snapshot_open(db, &snap_db2, FDB_SNAPSHOT_INMEM);
  TEST_CHK(s == FDB_RESULT_SUCCESS);
  ...
  s = fdb_snapshot_open(db, &snap_db3, FDB_SNAPSHOT_INMEM);
  TEST_CHK(s == FDB_RESULT_SUCCESS);
  ...
  for (i=0;i<n;++i){
      sprintf(keybuf, "%0*d", 8, i);
      sprintf(bodybuf, "body%04d", i);
      s = fdb_get_kv(snap_db, keybuf, strlen(keybuf), &value, &value_len);
      TEST_CHK(s == FDB_RESULT_SUCCESS);
  }
  ...
  for (i=0;i<n;++i){
      sprintf(keybuf, "%0*d", 8, i);
      sprintf(bodybuf, "body%04d", i);
      s = fdb_get_kv(snap_db2, keybuf, strlen(keybuf), &value, &value_len);
      TEST_CHK(s == FDB_RESULT_SUCCESS);
  }

  s = fdb_kvs_close(snap_db2);
  TEST_CHK(s == FDB_RESULT_SUCCESS);
  ...
  for (i=0;i<n;++i){
      sprintf(keybuf, "%0*d", 8, i);
      sprintf(bodybuf, "body%04d", i);
      s = fdb_get_kv(snap_db3, keybuf, strlen(keybuf), &value, &value_len);
      TEST_CHK(s == FDB_RESULT_SUCCESS);
  }
  ...
  s = fdb_shutdown();
  // TEST_CHK(s == FDB_RESULT_SUCCESS);
  TEST_RESULT("compact with snapshot_open read test");
}
...
void compact_upto_with_circular_reuse_test()
{
    ...
    s = fdb_compact_upto_kvssd(dbfile, markers_out[5].marker);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
...
void compact_upto_last_wal_flush_bid_check()
{
    ...
    s = fdb_compact_upto_kvssd(dbfile, markers_out[15].marker);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
...
/*
    In ForestKV, we perform stale key deletion during WAL flushes.
    In one thread, start a WAL flush. In another, start compaction
    (stale key deletion). They shouldn't be able to trigger
    stale key deletion at the same time, and no errors should
    occur.
*/
...
void * wal_flusher(void* voidargs)
{
    struct t_args *args;
    args = (struct t_args*) voidargs;

    while(!args->start) {}

    fdb_commit(args->dbfile, FDB_COMMIT_MANUAL_WAL_FLUSH);
...
void * compactor(void* voidargs)
{
    ...
    fdb_compact_kvssd(args->dbfile);

    return 0;
}

void compact_during_wal_flush_test()
{
    TEST_INIT();
    memleak_start();
    _reset_kvssd("/dev/nvme0n1", "forestKV");
    ...
    fdb_status s;
    char keybuf[256];
    thread_t flusher_tid, compactor_tid;
    struct t_args *args;
    std::condition_variable start_cond;
    uint8_t start = 0;
    ...
    */
    ...
    /*
        Second insert, makes stale keys
    */
    ...
    args = (struct t_args*)malloc(sizeof(*args));
    args->dbfile = dbfile;
    args->start_cond = &start_cond;
    args->start = &start;

    thread_create(&flusher_tid, wal_flusher, args);
    thread_create(&compactor_tid, compactor, args);

    start = 1;

    thread_join(flusher_tid, NULL);
    thread_join(compactor_tid, NULL);
    ...
    fdb_shutdown();
    memleak_end();
    TEST_RESULT("compact_during_wal_flush_test test");
}

int main(){
    large_insert_compaction_test();
    ...
    compact_with_snapshot_open_read_test();
    compact_with_snapshot_open_test();
    compact_during_wal_flush_test();
