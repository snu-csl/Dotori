...
void rollback_secondary_kvs()
{
    ...
    // seq:2
    status = fdb_set_kv(kv1, (void *) "aaaaaaaa", 8, (void *)"val-a", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_set_kv(kv1, (void *) "bbbbbbbb", 8, (void *)"val-b", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    fdb_commit(dbfile, FDB_COMMIT_MANUAL_WAL_FLUSH);

    // seq:3
    status = fdb_set_kv(kv1, (void *) "bbbbbbbb", 8, (void *)"val-v", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    fdb_commit(dbfile, FDB_COMMIT_MANUAL_WAL_FLUSH);

    // seq:4
//    status = fdb_del_kv(kv1, (void *)"aaaaaaaa", 8); // no delete currently
    status = fdb_set_kv(kv1, (void *) "aaaaaaaa", 8, (void *)"val-d", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    fdb_commit(dbfile, FDB_COMMIT_MANUAL_WAL_FLUSH);

    // get 'a' via kv2
    status = fdb_get_kv(kv2, (void *)"bbbbbbbb", 8, &value_out, &valuelen_out);
    free(value_out);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
void snapshot_test()
{
    ...
    *(char *)doc[i]->body = 'b';
    ...
    status = fdb_set_kv(db, (void *) "aaaaaaaa", 8, (void *)"val-a", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_commit(dbfile, FDB_COMMIT_NORMAL);
    TEST_CHK(status == FDB_RESULT_SUCCESS);

    // 3. Create Key 'b' and Commit
    status = fdb_set_kv(db, (void *)"bbbbbbbb", 8, (void *)"val-b", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_commit(dbfile, FDB_COMMIT_NORMAL);
    TEST_CHK(status == FDB_RESULT_SUCCESS);

    // 4. Create Key 'c' and Commit
    status = fdb_set_kv(db, (void *)"cccccccc", 8, (void *)"val-c", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_commit(dbfile, FDB_COMMIT_NORMAL);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    ...
    status = fdb_iterator_init(db, &iterator, (void *)"aaaaaaaa", 8,
                               (void *)"cccccccc", 8, FDB_ITR_NONE);
    TEST_CHK(status == FDB_RESULT_SUCCESS);

    // 7. Do a seek
    status = fdb_iterator_seek(iterator, (void *)"bbbbbbbb", 8, FDB_ITR_SEEK_HIGHER);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    ...
    status = fdb_iterator_init(snap_db, &iterator, (void *)"aaaaaaaa", 8,
                               (void *)"cccccccc", 8, FDB_ITR_NONE);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
// dubious usefulness with only one KVS
void snapshot_with_uncomitted_data_test()
{
    ...
    s = fdb_kvs_open_default(db_file, &db0, &kvs_config);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
...
void in_memory_snapshot_on_dirty_hbtrie_test()
{
    ...
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    s = fdb_kvs_open_default(db_file, &db, &kvs_config);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    ...
    for (i=0;i<n/10;++i){
        ...
        memcpy(value + value_len - 6, "<end>", 6);
        sprintf(value, valuestr, idx);
        s = fdb_set_kv(db, key, strlen(key), value, value_len);
        TEST_CHK(s == FDB_RESULT_SUCCESS);
    }
    ...
    idx = n/10;
    sprintf(key, "%0*d", 8, idx);
    s = fdb_get_kv(snap, key, strlen(key), &v_out, &vlen_out);
    // should not be able to retrieve
    TEST_CHK(s != FDB_RESULT_SUCCESS);

    s = fdb_get_kv(snap_clone, key, strlen(key), &v_out, &vlen_out);
...
void in_memory_snapshot_compaction_test()
{
    ...
    s = fdb_kvs_open_default(db_file, &db2, &kvs_config);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    ...
    s = fdb_get_kvs_info(snap, &info);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    TEST_CHK(info.last_seqnum == (fdb_seqnum_t)n);
    ...
    s = fdb_get_kv(snap, (void*)key, strlen(key), &value_out, &valuelen);
    TEST_CHK(s == FDB_RESULT_SUCCESS);
    TEST_CMP(value_out, value, valuelen);
    fdb_free_block(value_out);
...
void rollback_and_snapshot_test()
{
    ...
    void *val;
    size_t vallen;
    status = fdb_get_kv(db, (void *)"cccccccc", 8, &val, &vallen);
    TEST_CHK(status == FDB_RESULT_KEY_NOT_FOUND);
    ...
    status = fdb_get_kv(snapshot, (void *)"cccccccc", 8, &val, &vallen);
    TEST_CHK(status == FDB_RESULT_KEY_NOT_FOUND);
...
static void _snapshot_check(fdb_kvs_handle *handle, int ndocs, int nupdates)
{
    ...
    for (i=0; i<nupdates; i++) {
        ...
        for (j=0;j<ndocs;++j) {
            ...
            s = fdb_get_kv(snap, keybuf, strlen(keybuf),
                           (void**)&value, &valuelen);
            TEST_CHK(s == FDB_RESULT_SUCCESS);
            TEST_CMP(value, bodybuf, valuelen);
            fdb_free_block(value);
...
void auto_compaction_snapshots_test()
{
    ...
    for(i=0;i<100000;i++) {
        ...
        if (i % 10 == 0) {
            ...
            *  If we don't manual WAL flush here, the fdb_get_byseq
            *  checks below will fail as we don't flush the WAL
            *  during compaction
            */
...
void in_memory_snapshot_rollback_test()
{
    ...
    status = fdb_set_kv(kvs, (void *)"key44444", 8, (void*)"value", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_set_kv(kvs, (void *)"key55555", 8, (void*)"value", 5);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
    status = fdb_commit(file, FDB_COMMIT_MANUAL_WAL_FLUSH);
    TEST_CHK(status == FDB_RESULT_SUCCESS);
...
void drop_kv_on_snap_iterator_test(){
    ...
    fdb_kvs_open_default(f1, &kv, &kvs_config);

    // write 2 seqno's
    status = fdb_set_kv(kv, (void *) "aaaaaaaa", 8, NULL, 0);
    TEST_STATUS(status);
    status = fdb_set_kv(kv, (void *) "bbbbbbbb", 8, NULL, 0);
    TEST_STATUS(status);
    ...
    // drop kvs
    status = fdb_kvs_remove(f1, "kvkvkvkv");
    TEST_CHK(status != FDB_RESULT_SUCCESS);
...
int main(){
    ...
    multi_version_test();
#ifdef __CRC32
    crash_recovery_test(true); // need to make these more KVSSD specific
    ...
    snapshot_stats_test();
    snapshot_with_uncomitted_data_test(); // doesn't work with cache on
    snapshot_markers_in_file_test(false); // single kv instance mode
    snapshot_with_deletes_test();
    snapshot_without_seqtree(false); // single kv instance mode // could use this to check updates
...
// rollback_prior_to_ops(true); // multi kv
// rollback_prior_to_ops(false); // multi kv
// rollback_forward_seqnum(); // multi kv
...
//    //rollback_during_ops_test(NULL); // << failing
//    //tx_crash_recover_test(); // << failing
//    //rollback_drop_multi_files_kvs_test(); // << not passing
